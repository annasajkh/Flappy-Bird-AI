...
public class Matrix implements Serializable, Cloneable
{
	 ...
	 * 
	 * @param matrix The other matrix to add to this
	 * 
	 ...
	 * matching elements.
	 * 
	 * @param matrix The matrix to multiply from this
	 * 
	 * @return A new Mat with the multiplication of the parameter
	 */
	public Matrix elementMult(final Matrix matrix)
	{
		return new Matrix(data).map(new MatFunc()
		{
			@Override
	 ...
	 */
	public Matrix mult(final Matrix matrix)
	{
		if(cols != matrix.rows) throw new RuntimeException("Rows don't match columns");
		
		return new Matrix(rows, matrix.cols).map(new MatFunc()
		{
			@Override
			public double perform(double val, int r, int c)
			{
	 ...
	 * multiple operations on each single element in the {@link Matrix}.
	 */
